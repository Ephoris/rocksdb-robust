...
{
    spdlog::trace("Picking compaciton at level {}", level_id + 1);
    rocksdb::ColumnFamilyMetaData cf_meta;
    db->GetColumnFamilyMetaData(&cf_meta);

    std::vector<std::string> input_file_names;

    for (auto &file : cf_meta.levels[level_id].files)
    {
        if (file.being_compacted) {continue;}
        input_file_names.push_back(file.name);
    }

    // TODO : Find out the correct way to calculate file limiting size per level 
    // this->rocksdb_compact_opt.output_file_size_limit = 64 << 20; //> 64 MiB

    return new CompactionTask(
        db, this, cf_name, input_file_names, level_id + 1, this->rocksdb_compact_opt, level_id, false, false);
}


void FluidLSMCompactor::OnFlushCompleted(rocksdb::DB * db, const ROCKSDB_NAMESPACE::FlushJobInfo & info)
{
    spdlog::trace("Running Flush Subroutine");
    rocksdb::ColumnFamilyMetaData cf_meta;
    db->GetColumnFamilyMetaData(&cf_meta);
    ...
    for (int level_idx = 0; level_idx < largest_level_idx; level_idx++)
    {
        ...
        bool level_1_needs_compact = ((level_idx == 0) && (live_runs >= this->fluid_opt.size_ratio));
